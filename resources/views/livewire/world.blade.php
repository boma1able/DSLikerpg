<div>
    ...
    <div class="flex border-3 border-[#e3e3e0]" style="gap: 20px; background: url({{ asset('storage/bg.jpeg') }}) top left no-repeat;background-size: cover; padding-right: 20px;">
        <div class="flex wfull max-w-[300px]">
                ...
                @foreach ($monsters as $monster)
                    @if (-$monster['position_x'] === $offsetX && -$monster['position_y'] === $offsetY)
                        <div wire:click="startBattle({{ $monster['id'] }})" class="group" style="position:relative;">
                            ...
                            <div class="flex relative bg-[#6767677d] text-[8px] text-black">
                                <span class="relative z-1 w-full h-[8px] items-center" style="line-height: 12px; font-weight: 600"></span>
                                <span style="position: absolute; top: 0; left: 0; width: {{ ($monster['max_mana'] > 0 && $monster['mana'] > 0) ? ($monster['mana'] / $monster['max_mana']) * 100 : 0 }}%; background: linear-gradient(90deg, #63adfc 0%, #322dda 70%, #02356b 90%); height: 100%;"></span>
                            </div>
                            <div class="absolute w-auto left-[100%] top-1 border border-gray-400 bg-white p-2 text-xs transition-transform duration-0 opacity-100 z-[-1] invisible group-hover:opacity-100 group-hover:z-1 group-hover:visible pointer-events-none">
        ...
        <div class="flex flex-wrap w-[650px]">
            ...
            <div class="w-full">
                ...
                <div class="w-full flex">
                    <div class="w-full min-w-[300px] h-[250px] overflow-hidden" style="border: 3px solid #eee;">
                    ...
                    <div class="w-full" style="min-width: 250px; height: 250px;position:relative; overflow:hidden; background: #01978f; border: 5px solid #eee;">
                                    ...
                                    @foreach ($row as $x => $tile)
                                        ...
                                        >
                                            @if (-$x === $offsetX && -$y === $offsetY)
                                                <span class="absolute opacity-50 w-[24px] h-[24px] p-2 overflow-hidden"
{{--                                                      style="background: radial-gradient(circle, rgb(255, 255, 255) 30%, rgb(255, 255, 255, .5) 70%, rgba(255, 255, 255, 0) 100%);">--}}
                                                      style="background: #555; border-radius: 50%;">
                                                </span>
                                            @endif

                                            @if ($count = collect($monsters)->where('position_x', $x)->where('position_y', $y)->count())
                                                <span class="relative text-xl">👹
                                                    <span class="absolute left-0 top-0 bg-white text-[8px]">{{ $count > 1 ? 'x' . $count : '' }}</span>
                                                </span>
                                            @endif
                ...
                <div wire:poll="moveMonsters"></div>
            ...
            <div class="flex flex-wrap w-full">
                {{--Log/chat--}}
                <div id="log-container" class="w-full h-[120px] mt-4 bg-gray-100 border border-gray-300 overflow-y-auto"
